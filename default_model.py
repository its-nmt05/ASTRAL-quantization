...
class AstralQuantizer(torch.nn.Module):
    ...
    def forward(self, waves_16k, wave_16k_lens):
    ...
    def decode(self, waves_16k, wave_16k_lens):
        assert waves_16k.size(0) == 1, "Batch size must be 1 for decoding"
        x_quantized, indices, feature_lens = self.forward(waves_16k, wave_16k_lens)
        assert self.asr_decoder is not None, "ASR decoder is not loaded"
        token_int = self.asr_decoder.decode(x_quantized)
        generated_text = self.tokenizer.decode(token_int, skip_special_tokens=True)
